...
def imageInfo(image, title="image"):
    ...
    if type(image) == numpy.ndarray:
        ...
        print "{0}: {1} not recognized as a numpy array".format(title, type(image))    
...
def imageShow(image, title="image", norm=True, wait=1):
    ...
    # CV_WINDOW_NORMAL is not in cv2, but it still works
    # to create a resizable window.
...
def montage(imgs, cols=4, space=5, norm=True):
    ...
    rows = int(numpy.ceil(float(d) / cols))
...
def labelPoints(image, pointsX, pointsY):
    ...
    locations. A copy of the image is created for non-destructive labeling.
    ...
    (h, w) = imgcopy.shape[0:2]
    radius = int(numpy.ceil(h / 100.0))
        
    for pX, pY in zip(pointsX, pointsY):
        # this draws assuming an RGB image
        cv2.circle(imgcopy, (int(pX), int(pY)), radius, (255, 0, 0))
