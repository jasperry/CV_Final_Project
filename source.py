...
class CameraCV(ImageSource):
    ...
    def generateData(self):
        ...
        if self._camera is not None:
            cvimg = self._camera.read()[1]
            output = cvimg[..., ::-1].astype(numpy.uint8)
        else:
            # test image if camera doesn't work
            output = numpy.zeros((480,640,3), numpy.uint8)
...
class VideoCV(ImageSource):
    ...
    def getLength(self):
        ...
        if self._length == 0 and self._video is not None:
            self.length = self._video.get(cv.CV_CAP_PROP_FRAME_COUNT)
    ...
    def getVideoFPS(self):
        ...
        if self._video is not None:
            fps = self._video.get(cv.CV_CAP_PROP_FPS)
        return fps
    ...
    def generateData(self):
        ...
        if self._lastIndex is not None and self._lastIndex != self.getIndex() - 1:
            retval = self._video.set(cv.CV_CAP_PROP_POS_FRAMES, self.getIndex())
...
def testVideoCV(args):
    import sink
    
    if len(args) < 2:
        print "Usage: python " + args[0] + " filename.mov"
        exit(0)
    
    # open vidcv and setup display
    video = VideoCV(args[1])
    video.setPlayMode(video.play)
    video.setLoop(True)
    view = sink.DisplayCV(video.getOutput())
        
    while cv2.waitKey(10) != 27:
        video.updatePlayMode()
        view.update()
        print("{0:4} of {1:4}: {2}, {3:6.2f} fps".format(video.getIndex(), video.getLength(), video.getFrameName(), video.getDataFrameRate()))
...
def testCameraCV():
    ...
    while cv2.waitKey(10) != 27:
        video.updatePlayMode()
        view.update()
        print("{0:4}: {1}, {2:6.2f} fps".format(video.getIndex(), video.getFrameName(), video.getDataFrameRate()))
...
def testCameraFW():
    ...
    while cv2.waitKey(10) != 27:
        video.updatePlayMode()
        view.update()
#        imgutil.imageInfo(camfw.getOutput().getData(), "image data")
        i += 1
        if i % 25 == 0:
            print "\tFPS: {0:8.3f}".format(float(video.getIndex()) / (time.time() - t0)), "\t", video.getIndex()

def testWriteCameraFW():
    import sink
    
    # open camera
    camfw = CameraFW(0, videoMode=FirewireVideo.DC1394_VIDEO_MODE_FORMAT7_0, shutter=2200, demosaic=False)
    camfw.setUpdateFramerate(20)
    
    # create writer
    writer = sink.FileWriter(camfw.getOutput())
    
    # save images
    for i in range(5):
        writer.setFilename("output-{0:04d}.raw".format(i))
        camfw.modified()
        writer.update()
